...
function launchLocator() {
    ...
    gfsh -e "start locator --security-properties-file=${APP_HOME}/etc/gfsecurity-one-way.properties --initial-heap=${DEFAULT_LOCATOR_MEMORY} --max-heap=${DEFAULT_LOCATOR_MEMORY} ${DEFAULT_JVM_OPTS} --name=locator$1_`hostname` --port=1033$1 --dir=${APP_HOME}/data/locator$1 --locators=${LOCATORS} --classpath=${APP_HOME}/etc --J=-Dgemfire.security-manager=org.apache.geode.examples.security.ExampleSecurityManager" &
...
function launchServer() {
    ...
    gfsh -e "connect --locator=${LOCATORS} --security-properties-file=${APP_HOME}/etc/gfsecurity-one-way.properties" -e "start server --locators=${LOCATORS} --security-properties-file=${APP_HOME}/etc/gfsecurity-one-way.properties --server-port=4040${1} --J=-Xmx${DEFAULT_SERVER_MEMORY} --J=-Xms${DEFAULT_SERVER_MEMORY} ${DEFAULT_JVM_OPTS} --name=server${1}_`hostname` --dir=${APP_HOME}/data/server${1} ${STD_SERVER_ITEMS} " &
...
gfsh -e "connect --locator=${LOCATORS}  --security-properties-file=${APP_HOME}/etc/gfsecurity-one-way.properties" -e "configure pdx --read-serialized=true --disk-store=DEFAULT"
...
gfsh -e "connect --locator=${LOCATORS} --security-properties-file=${APP_HOME}/etc/gfsecurity-one-way.properties" -e "create region --name=test --type=PARTITION"
